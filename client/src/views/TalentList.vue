  ...
  <div>
    ...
    <el-card shadow="never" style="margin-top:16px">
      ...
      <el-table :data="talents" v-loading="loading" stripe style="width:100%" @row-click="openDetail" class="talent-table">
        ...
        <el-table-column label="个人链接" width="130" v-if="visibleCols.includes('personal_links')">
          <template #default="{ row }">
            <div v-if="row.linkedin_url || row.github_url || row.maimai_url || row.homepage" class="link-icons">
              <el-tooltip v-if="row.linkedin_url" content="LinkedIn" placement="top">
                <a :href="row.linkedin_url" target="_blank" class="link-icon-btn linkedin-icon" @click.stop>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="#0A66C2"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
              </el-tooltip>
              <el-tooltip v-if="row.github_url" content="GitHub" placement="top">
                <a :href="row.github_url" target="_blank" class="link-icon-btn github-icon" @click.stop>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="#24292f"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>
                </a>
              </el-tooltip>
              <el-tooltip v-if="row.maimai_url" content="脉脉" placement="top">
                <a :href="row.maimai_url" target="_blank" class="link-icon-btn maimai-icon" @click.stop>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="#2088FF"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-6h2v6zm4 0h-2v-6h2v6zm-2-8a1 1 0 110-2 1 1 0 010 2zm-4 0a1 1 0 110-2 1 1 0 010 2z"/></svg>
                </a>
              </el-tooltip>
              <el-tooltip v-if="row.homepage" content="个人主页" placement="top">
                <a :href="row.homepage" target="_blank" class="link-icon-btn homepage-icon" @click.stop>
...
import { ArrowUp, ArrowDown, HomeFilled } from '@element-plus/icons-vue'
...
const visibleCols = ref(['company', 'title', 'location', 'education', 'tags','personal_links','skills' ])
...
.advanced-search { }
.link-icons { display: flex; align-items: center; gap: 6px; }
.link-icon-btn {
  display: inline-flex;
  align-items: center;
...
.link-icon-btn.linkedin-icon:hover { background: rgba(10,102,194,0.1); }
.link-icon-btn.github-icon:hover { background: rgba(36,41,47,0.1); }
.link-icon-btn.maimai-icon:hover { background: rgba(32,136,255,0.1); }
.link-icon-btn.homepage-icon { color: #606266; }
.link-icon-btn.homepage-icon:hover { color: #409eff; background: rgba(64,158,255,0.1); }
</style>
